...
class _RequestScreenState extends ConsumerState<RequestScreen> {
      ...
      if (pos != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                '🎯 تم لقط موقع بيتك بالـ GPS بنجاح! سينتقل الفني لعنوانك بدقة متناهية.',
              ),
  ...
  Future<void> _loadSavedClientData() async {
    try {
      ...
      if (savedPhone.isNotEmpty &&
          (verifiedPhone == null || verifiedPhone.isEmpty)) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final normalizedVerifiedPhone = _normalizePhone(
        prefs.getString('verified_phone') ??
            prefs.getString('client_phone') ??
            '',
      );

      final isAlreadyVerified =
          normalizedInputPhone.isNotEmpty &&
          (normalizedVerifiedPhone == normalizedInputPhone ||
              prefs.getBool('is_client_verified') == true);
      ...
      final discountAmount = _appliedPromo != null
          ? _appliedPromo!.calculateDiscount(100)
          : 0;
      final order = Order(
        id: '',
        trackingCode: '',
        ...
        status: _preSelectedTechId != null
            ? OrderStatus.assigned
            : OrderStatus.pending,
        promoCode: _appliedPromo?.code,
        discountAmount: discountAmount,
        isScheduled: _isScheduled,
        scheduledDate: _isScheduled
  ...
  void _showTechUnavailableDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'عذراً، الفني الذي اخترته لم يعد متاحاً لاستقبال طلبات في هذه اللحظة. يمكنك إرسال الطلب كـ "طلب عام" وسيقوم أفضل فني متاح في منطقتك بالتواصل معك.',
        ),
        actions: [
          TextButton(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              'كود التتبع: ${result.trackingCode}',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'طلبك محمي بضمان حرفي لمدة شهر 🛡️',
                ...
                Text(
                  'تأكيد الخدمة من التطبيق يُفعّل لك الضمان تلقائياً.',
          ...
          return Padding(
            ...
            child: SizedBox(
              ...
              child: _OrganicServiceItem(
                ...
                onTap: () {
                  ref.read(requestSelectedServiceProvider.notifier).state =
                      type;
                  setState(() => _preSelectedTechId = null);
                },
              ),
    ...
    return techsAsync.when(
      ...
      data: (techs) {
        final filtered = techs
            .where((t) => t.spec == service && t.canAcceptOrders)
            .toList();
        ...
        if (filtered.isEmpty) {
          return Container(
            ...
            child: Text(
              'سيتم تعيين أفضل فني متاح لك فور إرسال الطلب.',
        ...
        return SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) => _TechOrganicMiniCard(
              tech: filtered[index],
              onSelect: () =>
                  setState(() => _preSelectedTechId = filtered[index].id),
            ),
          ),
        );
      },
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isEligible ? tech.rank : 'غير متاح حالياً',
                      style: AppTextStyles.labelMed.copyWith(
                        color: isEligible ? AppColors.gold : AppColors.error,
                      ),
                    ),
                  ],
                ),
        ...
        children: [
          AppTextField(
            ...
            validator: (v) {
              ...
              if (parts.length < 2)
                return 'يرجى كتابة اسم ثنائي على الأقل (مثال: أحمد علي)';
              return null;
            },
          ),
          const SizedBox(height: 16),
          ...
          AppTextField(
            ...
            validator: (v) {
              ...
              if (v.trim().length < 5)
                return 'أدخل عنواناً واضحاً (أكثر من 5 أحرف)';
              return null;
            },
          ),
          ...
          OutlinedButton.icon(
            style: OutlinedButton.styleFrom(
              foregroundColor: _selectedLat != null
                  ? AppColors.success
                  : AppColors.gold,
              side: BorderSide(
            ...
            icon: _isDetectingLocation
                ...
                : Icon(
                    _selectedLat != null ? Icons.my_location : Icons.gps_fixed,
                    size: 18,
                  ),
            label: Text(
              _selectedLat != null
          ...
          AppTextField(
            ...
            validator: (v) {
              if (v == null || v.trim().isEmpty)
                return 'يرجى كتابة وصف مختصر للمشكلة';
              if (v.trim().length < 5)
                return 'صف المشكلة بوضوح أكثر ليساعد الفني (5 أحرف على الأقل)';
              return null;
            },
          ),
        ],
  ...
  Future<void> _applyPromoCode() async {
    ...
    try {
      ...
      if (promo == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم تطبيق كود الخصم (${promo.code}) بنجاح! 🎉'),
          ),
        );
      }
    } catch (_) {
      ...
      child: AnimatedContainer(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            ...
            children: [
              Image.asset(
                'assets/images/${_getAsset(type)}',
                fit: BoxFit.cover,
                errorBuilder: (c, e, s) => Container(color: AppColors.surface2),
              ),
              Container(
                decoration: BoxDecoration(
